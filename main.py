...
def home():
    return "Welcome to Digital Airlines API"
...
def register():
    ...
    # If no one found with same email or username
    if found == 0:
...
# Admin login can be done with the same endpoint for the users

# The same goes with the logout

# def checkAdminAuthentication(request):
#     userId = request.cookies.get("userID")
#     admin = usersCollection.find_one({"_id": ObjectId(userId)})
#     if admin:
#         if (
#             admin["username"] == "admin"
#             and admin["surname"] == "admin"
#             and admin["email"] == "admin@gmail.com"
#             and admin["password"] == "admin123"
#         ):
#             return True
#     return False
...
def createFlight():
    ...
    # If id is not None the admin is authenticated
    if adminID:
...
def getFlightInfo():
    try:
        ...
        if adminID:
            ...
            flight = flightsCollection.find_one(
                {"_id": ObjectId(flight_id)}, {"_id": 0}
            )
